...
1. [프로젝트 개요](#프로젝트-개요)
2. [주요 컬럼](#주요-컬럼)
3. [프로젝트 전체 프로세스](#프로젝트-전체-프로세스)
4. [Feature Engineering (핵심)](#feature-engineering-핵심)
5. [A 검사 (신규 자격검사) Feature Summary](#a-검사-신규-자격검사-feature-summary)
   - [터널 끝 지점 맞추기](#1-터널-끝-지점-맞추기)
   - [적색선 위 정지 (속도 반영)](#2-적색선-위-정지-속도-반영)
   - [화면 화살표 방향 반응](#3-화면-화살표-방향-반응)
   - [원 색깔 구분](#4-원-색깔-구분)
   - [그림판 BEFORE–AFTER 비교](#5-그림판-beforeafter-비교)
6. [B 검사 (자격 유지검사) Feature Summary](#b-검사-자격-유지검사-feature-summary)
   - [화면 시야 자극 찾기](#1-화면-시야-자극-찾기)
   - [시야 자극 찾기 2단계](#2-시야-자극-찾기-2단계)
   - [위험신호 → 즉시 브레이크](#3-위험신호--즉시-브레이크)
   - [화살표 방향과 동일한 버튼 누르기](#4-화살표-방향과-동일한-버튼-누르기)
   - [도로·지역·차량 찾기 항목](#5~8-도로-찾기--특정-지역-찾기67--특정-차량-찾기)
7. [Feature Selection](#feature-selection)
8. [모델링 전략](#모델링-전략)
   - [데이터셋 분할](#1-데이터셋-분할)
   - [모델 구조](#2-모델-구조)
   - [확률 보정을 위한 Calibration](#3-확률-보정을-위한-calibration)
9. [평가 지표](#평가-지표)
10. [운수종사자 교통사고 위험분석 결과 해석 & 인사이트](#운수종사자-교통사고-위험분석-결과-해석--인사이트)
...
## 운수종사자 교통사고 위험분석 결과 해석 & 인사이트

### 결과 해석 및 인사이트

본 데이터는 **사고 발생 여부 라벨의 불균형이 매우 심한 구조**를 가지고 있어,  
불균형 데이터에 강점을 가지는 모델을 비교 검토하였다.

- 기존 XGBoost 모델 외에  
  **Balanced Random Forest(balanced_rf)** 모델을 적용하여 학습을 수행하였으나,
- 실제 평가 지표(AUC, Brier Score, ECE) 기준에서는  
  오히려 성능이 저하되는 결과를 확인하였다.

본 과제는 확률 평가 지표를 중심으로 성능을 평가하는 문제로,  
balanced_rf의 class rebalancing 과정이 예측 확률 분포를 왜곡하여  
오히려 성능 저하로 이어졌을 가능성이 있다고 판단하였다.

또한,
- 정보 공유가 거의 없는 변수들을 대상으로  
  - **Cohen’s d** (사고/비사고 집단 간 효과 크기)
  - 다른 변수들과의 **상관성 분석**
  을 수행하여
- 설명력이 낮은 변수를 제거함으로써  
  **모델 학습 속도 및 안정성을 향상**시켰다.

최종적으로 XGBoost 모델에서 다음과 같은 하이퍼파라미터 튜닝을 수행하였다.
...
본 분석에서는  
**response 변수들만 전처리하여 모델 학습에 사용**하였다.

이로 인해,
- 실험 조건(condition)에 따른 반응 차이
- 운전 상황별 반응 패턴
등을 충분히 반영하지 못한 한계가 존재한다.

즉,  
사고 위험에 영향을 줄 수 있는 **조건 변수의 정보 활용이 제한적**이었다.

### 개선 방향

향후 분석에서는 condition 변수를 단순 범주형 변수로 사용하는 것을 넘어,  
조건별 반응 특성을 정량화한 파생 변수를 생성하는 것이 필요하다고 판단된다.
...
이와 같은 **조건 기반 파생 변수**를 추가할 경우,  
운전 상황별 위험 요인을 보다 정교하게 학습할 수 있으며  
모델의 일반화 성능 또한 추가로 개선될 것으로 기대된다.
